...
#define HZ_PROFILE 0
	...
	#define HZ_PROFILE_BEGIN_SESSION(name, filepath) ::BraveFish::Instrumentor::Get().BeginSession(name, filepath)
	#define HZ_PROFILE_END_SESSION() ::BraveFish::Instrumentor::Get().EndSession()
	#define HZ_PROFILE_SCOPE(name) constexpr auto fixedName = ::BraveFish::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
									::BraveFish::InstrumentationTimer timer##__LINE__(fixedName.Data)
